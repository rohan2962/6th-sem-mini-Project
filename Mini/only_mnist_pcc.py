...
for dig in range(10):
    ...
    for j in range(rang_mnist[dig][0],rang_mnist[dig][1],600):
        ...
        [x_mnist1,eigen1]=conv_pca(x_mnist1,50)
        ...
        xlc=xlc[0]
        xlc = np.dot(eigen1, xlc.T).T
        final_eig=copy.deepcopy(eigen1)
        final_k3=k3
        pdig=0
        xtr1=[]
        ytr1=[]
    ...
    print(np.array(x_mnist1).shape)
    print(np.array(new_Xtest).shape)
    one_class_svm(x_mnist1, new_Xtest, y_train, y_test, dig)
    ...
    plt.ylabel('Accuracy')
    #plt.show()
    plt.savefig('Acc vs Batches merged for Digit  '+str(dig)+'.png')
    plt.clf()
    plt.plot([i for i in range(len(no_of_fe))],no_of_fe,label='No of components after merging',marker='o')
    #print(k_2)
    ...
    plt.legend()
    #plt.show()
    plt.savefig('No of features vs Batches merged for Digit  '+str(dig)+'.png')
    plt.clf()'''
